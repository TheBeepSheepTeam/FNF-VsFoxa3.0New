...
class PlayState extends MusicBeatState
{
	...
	// this new option totally gets the job done
	 ...
	 */
		...
		switch (curIconSwing)
		{
			...
			// does nothing, that's not needed tbh
			default:
		...
		iconP1.x = healthBar.x
			+ (healthBar.width * (FlxMath.remapToRange(health * 50, 0, 100, 100, 0) * 0.01))
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		if (!cpuControlled
			...
			&& !paused
			&& key > -1
			&& (FlxG.keys.checkStatus(eventKey, JUST_PRESSED) || controls.controllerMode))
		{
