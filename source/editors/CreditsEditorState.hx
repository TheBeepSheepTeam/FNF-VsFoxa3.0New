...
class CreditsEditorState extends MusicBeatState
{
	...
	function addCreditsUI():Void
	{
		...
		titleJump.textX += 3;
		titleJump.textY += 4;
		if (FlxG.save.data.jumpTitle == null)
			FlxG.save.data.jumpTitle = true;
		...
		{
			var icon:String;
			if (iconInput.text != null && iconInput.text.length > 0)
				icon = iconInput.text;
			else
				icon = creditsStuff[curSelected][1];

			var pathIcon:String;
			if (Paths.fileExists('images/credits/' + icon + '.png', IMAGE))
				pathIcon = 'credits/' + icon;
			else
				pathIcon = 'credits/none';
			...
			if (isSelectable)
			{
				...
				else
				{
					...
					if (creditsStuff[i][1] == null || creditsStuff[i][1] == '')
						icon = new AttachedSprite('credits/none');
				}
	...
	function setItemData()
	{
		if (curSelIsTitle)
		{
			if (titleInput.text != null && titleInput.text.length > 0)
				creditsStuff[curSelected][0] = titleInput.text;
			else
				creditsStuff[curSelected][0] = 'Title';
		}
		else
		{
			if (creditNameInput.text != null && creditNameInput.text.length > 0)
				creditsStuff[curSelected][0] = creditNameInput.text;
			else
				creditsStuff[curSelected][0] = 'User';

			creditsStuff[curSelected][1] = iconInput.text;

			if (descInput.text != null && descInput.text.length > 0)
				creditsStuff[curSelected][2] = descInput.text;
			else
				creditsStuff[curSelected][2] = 'Description here...';
			...
			{
				creditsStuff[curSelected][4] = 'e1e1e1';
			}
		}
	}
	...
	function deleteSelItem()
	{
		...
		for (i in 0...creditsStuff.length)
		{
			...
			{
				var shit:Bool = true;
				if (nullCheck(curSelected - 1))
				{ // remove space betwen title's
	...
	{
		return ([['Title'], ['User', '', 'Description here...', '', 'e1e1e1']]);
	}
	...
	function showIconExist(text:String)
	{
		...
		{
			if (!Paths.fileExists('images/credits/' + text + '.png', IMAGE))
				daColor = Std.parseInt('0xFFFF004C'); // icon not found
			else
				daColor = Std.parseInt('0xFF00FF37'); // icon was found
		}
